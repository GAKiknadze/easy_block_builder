    ...
    def __init__(self, properties: dict[str, object]) -> None:
        ...
        if self._input_schema is not None:
            try:
                ...
                self._logger.debug(f"Validated properties against schema {self._input_schema.__name__}")
    ...
    async def build(self, ctx: Context) -> tuple[dict[str, object], Type[BaseModel] | None]:
        ...
        if self._output_schema is not None:
            try:
                ...
                self._logger.debug(f"Validated output against schema {self._output_schema.__name__}")
            except ValidationError as e:
